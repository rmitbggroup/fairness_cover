...
public class Initialization {
    ...
    //remove removeThreshold% billboards
    public void deleteBillboardPre() {
        System.out.println("Before remove --  Route Num " + routes.size());
        int totalInf = 0;
        for (Billboard billboard : billboards) {
            totalInf += billboard.getRoutes().size();
        }
        System.out.println("Before remove --  Total Inf " + totalInf);
        int size = billboards.size();
        int size2 = (int) (size * (1.0 - Setting.removeThreshold));

        while (size2 < billboards.size()) {
            totalInf -= billboards.get(0).getRoutes().size();
            billboards.remove(0);
        }

//        while (size2 < billboards.size()) {
//            totalInf -= billboards.get(billboards.size() - 1).getRoutes().size();
//            billboards.remove(billboards.size() - 1);
//        }
        System.out.println("After remove --  Total Inf " + totalInf);
        System.out.println("After remove --  Billboard Num " + billboards.size() + " (" + Setting.removeThreshold + ")");

        printTrajNumber();
    }

    private void printTrajNumber() {
        ...
        System.out.println("After remove --  Route Num " + routes.size());
    }
