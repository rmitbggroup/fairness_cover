...
public class BillboardSet {
    ...
    public String getBillboardList() {
        ...
        for (Billboard billboard : billboards) {
            int panelID = Integer.parseInt(billboard.panelID);
            result += panelID + "/";
        }
    ...
    public String getResultString() {
        ...
        double cost = 0;
        if (Setting.test) {
            // test model, show more details
            result += "Billboard: ";

            for (Billboard billboard : billboards)
                result += billboard.panelID + "(" + billboard.getRoutes().size() + ")" + ", ";
        }
        ...
        if (billboards.size() == 0 && routes.size() > 0)
