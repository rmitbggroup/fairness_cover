...
public class GreedyFunction {
    ...
    public Billboard greedyOneLazy(List<Billboard> remainingBillboards, int advID) {
        double optInf = 0;
        double margInf_opt = 0;
        index = -1;
        Billboard billboard;

        for (int i = 0; i < remainingBillboards.size(); i++) {
            billboard = remainingBillboards.get(i);
            double margInf = billboard.updateMargInf(advID);
            if (margInf == 1) {
                index = i;
                break;
            }
            if (margInf > margInf_opt) {
//                System.out.println("!! Update !!");
//                System.out.println("Index " + i);
//                System.out.println("Inf " + billboard.influence);
//                System.out.println("margInf " + margInf);
                margInf_opt = margInf;
                optInf = billboard.influence;
                index = i;
            } else if (margInf == margInf_opt && billboard.influence > optInf) {
//                System.out.println("!! Update !!");
//                System.out.println("Index " + i);
//                System.out.println("Inf " + billboard.influence);
//                System.out.println("margInf " + margInf);
    ...
    public void orderByCostPer(ArrayList<Advertiser> advertiserList) {
        ...
        for (Advertiser advertiser : advertiserList) {
            //costPer.add(advertiser.getRequiredInf());
            costPer.add(advertiser.getBudget() / advertiser.getRequiredInf());
            //costPer.add(paymentList.get(advID) / requiredInfList.get(advID));
        }
     ...
     * costPer = payment / required influence
     */
    public void orderByCostPerRough(ArrayList<Advertiser> advertiserList) {
        int advNum = advertiserList.size();
        ArrayList<Integer> costPer = new ArrayList<>();
        for (Advertiser advertiser : advertiserList) {
            costPer.add((int) (advertiser.getBudget() * 100 / advertiser.getRequiredInf()));
        }
        ...
        for (int i = 0; i < advNum - 1; i++) {
            for (int n = i + 1; n < advNum; n++) {
                if (costPer.get(i) < costPer.get(n)) {
                    Collections.swap(advertiserList, i, n);
                    Collections.swap(costPer, i, n);
                } else if (costPer.get(i).equals(costPer.get(n))) {
                    if (advertiserList.get(i).getBudget() < advertiserList.get(n).getBudget()) {
                        Collections.swap(advertiserList, i, n);
                        Collections.swap(costPer, i, n);
                    }
                }
            ...
            for (int index = 0; index < pickedBillboardLists.get(i).size(); index++) {
                for (Billboard billboard : remainingBoards) {
                    if (pickedBillboardLists.get(i).get(index).panelID.equals(billboard.panelID)) {
                        if (Setting.test2)
                            System.out.println("Find billboard " + billboard.panelID + ", Picked ("
                                    + ++index + "/" + pickedBillboardLists.get(i).size() + ")");
    ...
    public double getRegret(Advertiser advertiser, double inf) {
        ...
        if (inf >= advertiser.getRequiredInf()) {
            ...
            //regret += resultSet.getCost();    \\ do not consider the cost of billboards
        } else {
            //regret = advertiser.getBudget();
