...
public class GreedyAlgorithm {
    ...
    public void greedyForAllPlus() {
        ...
        while (remainingBoards.size() > 0 && !isFinish) {
            ...
            for (int i = 0; i < advertiserList.size() - releaseNum; i++) {
                ...
                if (remainingBoards.size() == 0) {
                    ...
                    //System.out.println("advNum - currentAdvNumSa - releaseNum : " + advNum + "-" + currentAdvNumSa + "-" + releaseNum + "=" + (advNum - currentAdvNumSa - releaseNum));
        ...
        if (Setting.test2) {
            currentAdvNumSa = 0;
            for (int i = 0; i < advertiserList.size(); i++) {
                double requiredInf = requiredInfList[i] - resultSetList[i].getInfluence(false);
                if (requiredInf <= 0) {
                    currentAdvNumSa++;
                    System.out.println("Adv " + i + " satisfied    " + requiredInf);
                } else {
                    System.out.println("Adv " + i + " un-satisfied " + requiredInf);
                }
            }
            System.out.println("Final state Total " + advertiserList.size() + "   Sa " + currentAdvNumSa + "  Jump " + releaseNum);
            System.out.println("Remaining Bill Num " + remainingBoards.size());
        }
    ...
    private void relaseAdv(int i) {
        ...
        //resultSetList[i] = new BillboardSet(resultSetList[i].getAdvID());
    }
