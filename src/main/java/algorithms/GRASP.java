...
public class GRASP {
    ...
    private boolean isUpdate; // wheather need to update the optPayment;
    ...
    public void findResult() {
        ...
        while (unUpdateCount < maxIteration) {
            ...
            if (Setting.test) {
                System.out.print(".");
                if (unUpdateCount % 100 == 0)
                    System.out.println("Un-update iteration " + unUpdateCount + "/" + maxIteration);
            }
        ...
        System.out.println("Current regret " + greedyFunction.getTotalRegret(advertiserList, optResultSetList));
    ...
    private void makeRCL() {
        if (rcl == null || rcl.size() != RCLSIZE) {
            rcl = new ArrayList<>();
            //greedyFunction.order(remainingBoards);
            //rcl = new ArrayList<>();
    ...
    private boolean update(BillboardSet[] resultSet) {
        //checkDoubleSelect(resultSet);
        //double revenue = getTotalPayment(resultSet);
        //double regret = getTotalRegret(resultSet);
        double regret = greedyFunction.getTotalRegretForce(advertiserList, resultSet);
//        if (revenue > optTotalPayment) {
//            optTotalPayment = revenue;
        if (regret < optTotalRegret) {
            ...
            if (Setting.test) {
                System.out.println("GRASP - Find better solution, Regret " + optTotalRegret);
            }
            return true;
        }
        return false;
